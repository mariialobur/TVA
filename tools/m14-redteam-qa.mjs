...
const engine=read('m14-engine.js');for(const n of ['function prep(q)','const order=shuffle(q.options.map((_,i)=>i))','c.correct=order.indexOf(q.correct)','final:{label:\'Final Pro · 52Q\'','pattern:[\'F\',\'M\',\'M\',\'D\']','pct>=75&&moduleFloor'])if(!engine.includes(n))fail('runtime exam invariant missing: '+n);if(engine.includes('function prep(q)')&&engine.includes('c.correct=order.indexOf(q.correct)'))ok('runtime option shuffle remaps correct answer');
...
const abs=/\b(toujours|jamais|automatiquement|uniquement|obligatoirement)\b/i;let longCorrect=0,absoluteLeak=0;for(const q of Q){const lens=q.options.map(x=>String(x).length),avg=lens.reduce((a,b)=>a+b,0)/4,cl=lens[q.correct];if(cl>avg*1.65&&cl-avg>22)longCorrect++;const distract=q.options.filter((_,i)=>i!==q.correct);if(!abs.test(q.options[q.correct])&&distract.filter(x=>abs.test(x)).length>=2)absoluteLeak++}
console.log('STYLE METRICS: correct-much-longer='+longCorrect+'; multi-absolute-distractors='+absoluteLeak);if(longCorrect>35)fail('too many length-signalled correct answers: '+longCorrect);if(absoluteLeak>28)fail('too many questions signalled by absolutist distractors: '+absoluteLeak);
